...
function GrindingScreen({
  ...
  const handleChoiceClick = (choiceStats) => {
    if (isProcessing || isLocked) return;
    setIsLocked(true);
    onChoiceSelect(choiceStats);
    setTimeout(() => {
      setIsLocked(false);
    }, 1500);
  };
        ...
        <>
          ...
          <div className="space-y-2 select-none">
            ...
            <div className="h-1.5 w-full bg-stone-900 rounded-full overflow-hidden border border-stone-800">
              <div
                className="h-full bg-gradient-to-r from-amber-600 to-amber-400 transition-all duration-500 will-change-transform"
          ...
          {/* Scenario Card */}
          <div className="bg-stone-900/80 border border-stone-800 hover:border-amber-900/60 p-6 sm:p-8 rounded-xl shadow-2xl backdrop-blur-sm space-y-4 transition-colors select-none">
            ...
            <div className="pt-4 space-y-3">
              ...
              {scenario.choices.map((choice, idx) => (
                <button
                  ...
                  onClick={() => handleChoiceClick(choice.stats)}
